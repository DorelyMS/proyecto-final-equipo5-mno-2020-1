...
Para problemas en general, la factorización de matrices busca descomponer una matriz *R* de dimensiones *m* $\\times$ *n* en dos matrices: $P \\in \\mathbb{R}^{k \\times m}$. La importancia de la descomposición de matrices se debe a que se busca aprender de la matriz *R* de tal forma en la que el producto de matrices $P^{\\rm T}*Q*$ consiga reducir la dimensionalidad de los datos y hacer más eficiente la predicción de nuevos datos. En este tipo de representación, las entradas de las matrices $r_{ij}*  indican para nuestro problema práctico la calificación que el usuario *i* le dio a la película *j*. El uso de la representación de este problema a través de matrices, con lleva al uso de dimensiones grandes, además de que éstas puedan ser singulares y suelen tener demasiadas entradas con ceros (ralas). Esto sucede porque prácticamente nadie ve todas las películas.
